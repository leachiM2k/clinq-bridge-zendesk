...
export async function createContact(
     ...
     * FAIL or UPDATE?
     */
    const response = await axios.post<IZendeskUpdateResponse>(`${parseEnvironment().ZENDESK_BASE_URL}/api/v2/users.json`, { user: vendorContact }, { headers: { Authorization: `Bearer ${accessToken}` } });
...
export async function updateContact(
    ...
    const response = await axios.put<IZendeskUpdateResponse>(`${parseEnvironment().ZENDESK_BASE_URL}/api/v2/users/${contact.id}.json`, { user: vendorContact }, { headers: { Authorization: `Bearer ${accessToken}` } });
...
export async function deleteContact(accessToken: string, id: string): Promise<void> {
    const response = await axios.delete<IZendeskUpdateResponse>(`${parseEnvironment().ZENDESK_BASE_URL}/api/v2/users/${id}.json`, { headers: { Authorization: `Bearer ${accessToken}` } });
...
export function getOAuth2RedirectUrl(): string {
    const { ZENDESK_CLIENT_ID, ZENDESK_REDIRECT_URL, ZENDESK_BASE_URL } = parseEnvironment();
    return ZENDESK_BASE_URL + '/oauth/authorizations/new?' + querystring.encode({
